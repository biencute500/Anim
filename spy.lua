...
-- (thêm dưới các khai báo hàm tiện ích, trước phần "main")
local function logServerToClient(remote, kind, args)
	print(kind == "event" and "=== SERVER FIRED REMOTE ===" or "=== SERVER INVOKED REMOTE ===")
	print("Remote:", remote:GetFullName())
	for i, val in ipairs(args) do
		print("Arg[" .. i .. "]:", val)
	end
	print(kind == "event" and "===========================" or "============================")

	-- đẩy vào log UI của SimpleSpy
	newRemote(
		kind,
		remote.Name .. " [S->C]",
		args,
...
local function attachServerListeners(obj)
	...
	elseif obj:IsA("RemoteFunction") then
		...
		obj.OnClientInvoke = function(...)
			...
			return nil -- bảo vệ game khỏi lỗi nếu server chờ giá trị trả về
		end
	end
end
